...
export default function PageHeader({ 
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        <h1 className="text-2xl font-semibold text-parrot-blue-dark">{title}</h1>
        {description && (
          <p className="text-parrot-gray-600 text-sm">{description}</p>
        )}
        {children}
      </div>
        ...
        <div className="flex gap-3">
          {actions.map((action, index) => {
            ...
            const buttonClass = actionConfig.isPrimary 
              ? "btn-parrot text-sm flex items-center gap-1" 
              : "btn-parrot-secondary text-sm flex items-center gap-1";
