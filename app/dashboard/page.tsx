    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="animate-pulse">
          <svg className="h-10 w-10 text-hubspot-orange" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-hubspot">
          ...
          {recentSubmissions.length === 0 ? (
            <div className="text-center py-8 border border-dashed border-hubspot-gray-300 rounded-hubspot bg-hubspot-gray-50">
              ...
              <p className="text-hubspot-gray-600">No submissions yet</p>
            </div>
          ) : (
            <div className="space-y-3">
              {recentSubmissions.map((submission) => (
                <div 
                  key={submission.id} 
                  className="flex items-center justify-between p-3 rounded-hubspot border border-hubspot-gray-200 hover:border-hubspot-gray-300 bg-white"
                >
                  <div>
                    <div className="font-medium text-hubspot-blue-dark">{submission.formName}</div>
                    <div className="text-sm text-hubspot-gray-600">{formatDate(submission.submittedAt)}</div>
                  </div>
        ...
        <div className="card-hubspot">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-medium text-hubspot-blue-dark">Your Forms</h2>
            <Link href="/dashboard/forms" className="text-hubspot-orange hover:text-hubspot-orange-dark text-sm font-medium">
              View All
            </Link>
          </div>
            ...
            <div className="space-y-3">
              {forms.map((form) => (
                <Link 
                  ...
                  className="flex items-center justify-between p-3 rounded-hubspot border border-hubspot-gray-200 hover:border-hubspot-orange transition-colors duration-200"
                >
                  <div className="flex items-center">
                    <div className="h-10 w-10 rounded-hubspot bg-hubspot-gray-100 text-hubspot-orange flex items-center justify-center mr-3">
                      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                    </div>
                    <div>
                      <div className="font-medium text-hubspot-blue-dark">{form.name}</div>
                      <div className="text-sm text-hubspot-gray-600">{getSubmissionsByForm(form.id).length} submissions</div>
                    </div>
                  </div>
